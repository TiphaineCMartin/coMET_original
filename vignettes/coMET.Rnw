...
are also provided on \url{http://www.epigen.kcl.ac.uk/comet}. Information about the package can viewed from within R using this command:
...
\begin{enumerate}
  \item info file is defined in the option DATA.FILE. \textcolor{red}{It is mandatory and has to be a file in tabular format with an header}.
  \item correlation file is defined in the option CORMATRIX.FILE. \textcolor{red}{It is mandatory and has to be a file in tabular format with an header}.
  \item extra info files are defined in the option DATA.FILE.LARGE. \textcolor{red}{It is not mandatory, but if you add it, it has to be a file in tabular format with an header}.
...
\textcolor{red}{It is mandatory and has to be a file in tabular format with an header}. The data file used for the correlation matrix is described in the option CORMATRIX.FILE. This tab-delimited file can take 3 formats described in the option CORMATRIX.FORMAT:
\begin{enumerate}
...
\textcolor{red}{It is a file where each line is one option. The name of option is in capital and is separated to its value by "=". If there are multiple values such as for the option LIST.TRACKS or the options for exta data}.
...
\begin{enumerate}
  ...
  \item you need change owner and the permission to access this folder. Only the user called Shiny can access.
