...
%\VignetteKeywords{Software, DifferentialMethylation, Visualization, Sequencing, Genetics, FunctionalGenomics, Microarray, MethylationArray, MethylSeq, ChIPSeq, DNASeq, RIPSeq, RNASeq, ExomeSeq, DNAMethylation, GenomeWideAssociation }
...
\author{Tiphaine C. Martin \footnote{tiphaine.martin@kcl.ac.uk}, Thomas Hardiman \footnote{thomas.hardiman@kcl.ac.uk}, Idil Yet \footnote{idil.yet@kcl.ac.uk}, Pei-Chien Tsai \footnote{peichien.tsai@kcl.ac.uk}, Jordana T. Bell \footnote{jordana.bell@kcl.ac.uk}}
...
CoMET uses the package called "psych" that is not under bioconductor. You need to install it before the installation of coMET
...
You can install also on the version R 3.1 via the master version of package on gitHub. You need to do the same thing that describe in the section "Install the development version of coMET from Bioconductor"
...
\begin{enumerate}
\item UcscTrack. Different UCSC tracks can be selected for visualisation from the table Browser of UCSC \url{http://genome-euro.ucsc.edu/cgi-bin/hgTables?hgsid=202842745\_Dlvit14QO0G6ZPpLoEVABG8aqfrm&clade=mammal&org=Human&db=hg19&hgta_group=varRep&hgta_track=cpgIslandExt&hgta_table=0&hgta\_regionType=genome&position=chr6%3A32726553\-32727053&hgta\_outputType=primaryTable&hgta\_outFileName=}
...
\\
\\
\centerline{\includegraphics[scale=.19]{../inst/extdata/JpegTables/ChromHMM}}
% 
% \begin{longtable}{|c|c|}
...
\\
\\
\centerline{\includegraphics[scale=.19]{../inst/extdata/JpegTables/structureBiomart}}
...
\\
\\
\centerline{\includegraphics[scale=.19]{../inst/extdata/JpegTables/ISCATrack}}

\subsection{miRNA Target Regions from Ensemb}
The colour of the miRNA target regions is defined by the R package Gviz.

\subsection{Binding Motif Biomart from Ensembl}

Listed on the next page are the different colours used for the different types of binding motifs. The frequency shown is that found in GRCh38 (hg38). Motifs with red text are found only in GRCh37 (hg19), motifs with blue text are found only in GRCh38 (hg38)
\\
\\
\centerline{\includegraphics[scale=0.80]{../inst/extdata/JpegTables/bindingMotifBiomart}}

\subsection{Other Regulatory Regions Biomart from Ensembl}

Listed bellow are the different colours used for the different types of regulatory regions. The frequency shown is that found in GRCh38 (hg38).
\\
\\
\centerline{\includegraphics[scale=0.19]{../inst/extdata/JpegTables/OtherRegulatoryRegions}}

\subsection{Regulatory Fetures Biomart from Ensembl}

Listed bellow are the different colours used for the different types of regulatory etures The frequency shown is that found in GRCh38 (hg38).
\\
\\
\centerline{\includegraphics[scale=0.19]{../inst/extdata/JpegTables/RegulatoryFeaturesBiomart}}

\subsection{Other Regulatory Segments Biomart from Ensembl}

Listed bellow are the different colours used for the different types of regulatory segments. The frequency shown is that found in GRCh38 (hg38). Segments with red text are found only in GRCh37 (hg19)
\\
\\
\centerline{\includegraphics[scale=0.19]{../inst/extdata/JpegTables/RegulatorySegmentsBiomart}}
